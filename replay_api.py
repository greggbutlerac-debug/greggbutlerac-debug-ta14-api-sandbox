            ...
            if total_bytes > MAX_REPLAY_PACKAGE_BYTES:
                raise HTTPException(
                    ...
                    detail={
                        ...
                        "message": (
                            "Replay package exceeds the configured "
                            "upload limit."
                        ),
                        "maximum_bytes": (
...
@router.get(
    ...
    description=(
        "Confirm that the independent replay-verification service "
        "is available."
    ),
)
...
@router.get(
    ...
    description=(
        "Return the public identity, capabilities, and boundary of "
        "the TA-14 Independent Route Replay Standard."
    ),
)
def replay_standard() -> dict[str, Any]:
    ...
    return {
        ...
        "boundary": (
            "Verification establishes integrity and correspondence "
            "of preserved records. It does not automatically prove "
            "the truth of an unauthenticated external source."
        ),
    }
...
@router.post(
    ...
    description=(
        "Upload a TA-14 replay ZIP package and independently verify "
        "its files, signatures, route identity, receipt dependencies, "
        "ledger, execution, and outcome."
    ),
    responses={
        200: {
            "description": (
        ...
        422: {
            "description": (
                "Package was processed but failed or remained "
                "incomplete under independent verification."
            )
        },
    },
)
async def verify_uploaded_replay_package(
    package: UploadFile = File(
        ...,
        description=(
            "TA-14 Independent Route Replay ZIP package."
        ),
    ),
) -> JSONResponse:
    """
    ...
    filename = _validate_upload_metadata(
        package
    )
    ...
    try:
        with tempfile.TemporaryDirectory(
            ...
            response_body = _report_response(
                report
            )
